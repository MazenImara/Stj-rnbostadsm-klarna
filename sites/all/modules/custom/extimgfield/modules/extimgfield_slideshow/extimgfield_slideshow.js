(function($) {
  Drupal.behaviors.extimgfield_slideshow = {
    attach: function(context) {

      for (i in Drupal.settings.extimgfield_slideshow) {
        var settings = Drupal.settings.extimgfield_slideshow[i],
          slideshow = $('div.' + i),
          num_slides = slideshow.children().length,
          $this = false;

        if (!slideshow.hasClass('extimgfield-slideshow-processed')) {
          slideshow.addClass('extimgfield-slideshow-processed');

          // Add padding if needed
          var max_outerWidth = 0;
          var max_outerHeight = 0;
          $('.extimgfield-slideshow-slide img', slideshow).each(function() {
            $this = $(this);
          ...
          $('.extimgfield-slideshow-slide a', slideshow).each(function() {
            $this = $(this);
            max_outerWidth = Math.max(max_outerWidth, $this.outerWidth(true));
            max_outerHeight = Math.max(max_outerHeight, $this.outerHeight(true));
          });
          $('.extimgfield-slideshow-slide', slideshow).each(function() {
            $this = $(this);
            max_outerWidth = Math.max(max_outerWidth, $this.outerWidth(true));
            max_outerHeight = Math.max(max_outerHeight, $this.outerHeight(true));
          });
          ...
          if (settings.pager != '') {
            ...
            if ((settings.pager == 'image' || settings.pager == 'carousel') && num_slides > 1) {
              ...
              if (settings.pager == 'carousel') {
                ...
                options.pager = "#" + i + "-carousel .extimgfield-slideshow-pager";
              }
            }
          }
          ...
          if (num_slides > 1) {
            ...
            if (settings.pager == 'number') {
              $('.extimgfield-slideshow-pager a').each(function(){
                $this = $(this);
      ...
      var rebuild_max_height = function(context) {
        var max_height = 0;
        var heights = $('.extimgfield-slideshow-slide',context).map(function ()
        {
          return $(this).height();
        }).get(),
      ...
      if (jQuery.isFunction($.fn.imagesLoaded)) {
        $('.extimgfield-slideshow').each(function() {
          $('img',this).imagesLoaded(function($images) {
            rebuild_max_height($images.parents('.extimgfield-slideshow'));
          });
        });
      }
      else {
