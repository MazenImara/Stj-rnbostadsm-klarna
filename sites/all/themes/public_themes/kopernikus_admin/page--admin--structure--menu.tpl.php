    ...
    <?php if (!empty($page['content'])) {
      if(current_path()==='admin/structure/menu') {
        if(module_exists('html_parser')) {
          $access = user_access('general_menu_add_custom_link');
          if(!empty($page['content']['system_main']['content']['main']['#markup']) && $html = str_get_html($page['content']['system_main']['content']['main']['#markup'])) {
	          $links = $html->find('a');
	          foreach($links as $link) {
	            $href = explode('/', $link->href);
	            if(array_pop($href)==='add') {
	              if(!$access) {
	                $td = $link->parent();
	                $td->outertext = '';
	              }
	          ...
	          $page['content']['system_main']['content']['main']['#markup'] = $html->save();
	          $html->clear();
	          unset($html);
					}
        }
